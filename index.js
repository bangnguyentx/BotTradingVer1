...
// --- HỆ THỐNG ADMIN & KEY ---
const ADMIN_IDS = ['7760459637']; // ID admin
const activationKeys = new Map(); // Lưu trữ keys: {type, created, expires, used, usedBy}
const subscribedUsers = new Map(); // Users đã kích hoạt: {userInfo, activatedAt, keyUsed, isAdmin}
...
async function runAutoAnalysis() {
    ...
    try {
        for (const coin of TARGET_COINS) {
            ...
            const dynamicDelay = 8000 + (Math.floor(analyzedCount / 10) * 1000) + (Math.random() * 2000); // 8-11+ giây
            ...
            try {
                ...
                if (coinError.message.includes('418') || coinError.message.includes('429')) {
                    ...
                    consecutiveErrors = 0; // Reset nếu lỗi không phải rate limit
                }
                continue;
            }
        }
...
async function analyzeAllCoins(chatId) {
    ...
    try {
        for (let i = 0; i < TARGET_COINS.length; i++) {
            ...
            await new Promise(r => setTimeout(r, 3000)); // Delay 3 giây mỗi coin
